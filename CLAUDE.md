...
- Process manager: pm2 (process name: `watchns`, port 3003 — 3001 is taken by Augmi on this box)
- Reverse proxy: nginx (`/etc/nginx/sites-enabled/watchns.conf` must `proxy_pass http://127.0.0.1:3003;`; editing it + `nginx -t && systemctl reload nginx` needs root)
