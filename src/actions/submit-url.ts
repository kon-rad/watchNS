...
async function insertVideoForCreator(
  ...
  const meta = await scrapeVideoMeta(platform as "youtube" | "tiktok" | "instagram", videoUrl);
  const embedUrl = getEmbedUrl(platform as "youtube" | "tiktok" | "instagram", videoUrl);
